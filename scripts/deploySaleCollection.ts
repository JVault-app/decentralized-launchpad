...
import { IcoSale, buildIcoSaleContentCell } from '../wrappers/IcoSale';
...
export async function run(provider: NetworkProvider) {
    ...
    const nftCollection = provider.open(IcoSale.createFromConfig({
        ownerAddress: provider.sender().address as Address,
        nextItemIndex: 0,
        collectionContent: buildIcoSaleContentCell(
            {
